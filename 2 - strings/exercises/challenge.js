function titleClass2(string) {
    const removeMp4 = string.slice(0, -4)
    const result = []

    for(let i = 0; i < removeMp4.length; i++) {
        if(removeMp4[i] >= 'a' && removeMp4[i] <= 'z' || removeMp4[i] >= 'A' && removeMp4[i] <= 'Zu') {
            result.push(removeMp4[i])
        }

        return removeMp4[7] >= 'a'
    }
}

console.log(titleClass2("01-01 Visão geral do capítulo.mp4"))
...
//     const videosObj = videos.map(video => {
//         const [id, title, duration] = video.split(",")
//         return { id, title: titleClass(title), duration }
//     })
